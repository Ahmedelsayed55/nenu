...
export const AssortedCakes = ({id}) => {
  ...
  return (
   <div>
      <h1 className="text-[16px] md:text-2xl mb-5">تورت مقاسات متنوعه</h1>
      <div
        className="container mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5"
        id={id}
      >
        {prduct.map((item) => {
          return (
            <div
              key={item.id}
              className="rounded-2xl shadow-yellow-500 shadow-sm p-5 flex flex-col gap-3 items-center transition hover:scale-110"
            >
